...
use bitvmx_wallet::{wallet::errors::WalletError, Destination};
...
enum TestAuxError {
    ...
    #[error("Failed with helper context")]
    FailedWithHelper {
        helper: TestHelper,
        #[source]
        source: anyhow::Error,
...
impl std::fmt::Debug for TestAuxError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            TestAuxError::General(e) => e.fmt(f),
            TestAuxError::BitVMXError(e) => e.fmt(f),
            TestAuxError::BrokerError(e) => e.fmt(f),
            TestAuxError::WalletError(e) => e.fmt(f),
            TestAuxError::FailedWithHelper { source, .. } => source.fmt(f),
        }
    }
}
...
fn test_aux(
    ...
    let msg = match helper.wait_msg(0) {
        Ok(m) => m,
        Err(e) => return Err(TestAuxError::FailedWithHelper { helper, source: e }),
    };
    info!("Setup dispute done: {:?}", msg);

    let msg = match helper.wait_msg(1) {
        Ok(m) => m,
        Err(e) => return Err(TestAuxError::FailedWithHelper { helper, source: e }),
    };
    info!("Setup dispute done: {:?}", msg);
...
fn test_wrong_public_input() {
    ...
    let Err(TestAuxError::FailedWithHelper { helper, .. }) = result else {
        panic!(
            "expected test_aux to fail with FailedWithHelper, got: {:?}",
            result
        );
    };

    let handle = helper.bitvmx_handle.unwrap();
    let err = handle.join().unwrap().unwrap_err();

    match err.downcast_ref::<BitVMXError>() {
        Some(BitVMXError::InvalidInput(msg)) => {
            assert_eq!(
                msg,
...
#[test]
#[ignore]
fn test_wrong_input() -> Result<(), TestAuxError> {
...
#[test]
#[ignore]
fn test_correct_input() -> Result<(), TestAuxError> {
...
#[test]
#[ignore]
fn test_sha512_correct_input() -> Result<(), TestAuxError> {
...
#[test]
#[ignore]
fn test_sha512_wrong_input() -> Result<(), TestAuxError> {
    let preimage = b"my_super_secret_string_that_noone_knowsAAAAAAAAAAAAAAAAAAAAAAAAA";
