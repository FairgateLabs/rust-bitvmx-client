...
### 1. `Backup` may hide storage failures
...
### 2. Wallet error classification is deferred
...
### 3. Inconsistent ZKP data can retry indefinitely
...
### 4. Malformed aggregated keys appear not ready
...
### 5. Permanent unhandled errors can block retries
...
1. Separate terminal backup request errors from storage/system failures.
2. Add wallet error classification and propagate wallet storage failures.
3. Decide whether inconsistent ZKP and malformed aggregated-key data are fatal corruption or terminal API failures.
4. Audit every remaining `Err` path to ensure permanent request errors cannot poison the retry queue.
5. Audit in-memory mutations and any dependency calls that may bypass the shared transactional storage.
6. Verify idempotency of broker consumers because transport delivery is at least once.
7. Update the README API response table for all newly documented `ApiError` outcomes.
