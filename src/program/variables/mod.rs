...
fn var_key(uuid: &Uuid, key: &str) -> StorageKey {
    StorageKey::new(["bitvmx", "var", uuid.to_string().as_str(), key])
}
...
fn witness_key(uuid: &Uuid, key: &str) -> StorageKey {
    StorageKey::new(["bitvmx", "witness", uuid.to_string().as_str(), key])
}
...
mod tests {
    ...
    use key_manager::{
        ...
        winternitz::{WinternitzPublicKey, WinternitzSignature, WinternitzType},
    };
    use std::str::FromStr;
