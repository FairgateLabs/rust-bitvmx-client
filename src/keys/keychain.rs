...
impl KeyChain {
    pub fn new(config: &Config) -> Result<KeyChain, BitVMXError> {
        ...
        let communications_key = Keypair::from_protobuf_encoding(&hex::decode(privk.as_bytes().to_vec()).unwrap()).unwrap();
    ...
    pub fn derive_keypair(&mut self) -> Result<PublicKey, BitVMXError> {
        self.key_manager.derive_keypair(self.ecdsa_index.next()).map_err(BitVMXError::from)
    }

    pub fn derive_keypair_with_index(&mut self, index: u32) -> Result<PublicKey, BitVMXError> {
        self.key_manager.derive_keypair(index).map_err(BitVMXError::from)
    }

    pub fn derive_winternitz_hash160(&mut self, message_bytes: usize) -> Result<WinternitzPublicKey, BitVMXError> {
        self.key_manager.derive_winternitz(
            message_bytes, 
    ...
    pub fn derive_winternitz_sha256(&mut self, message_bytes: usize) -> Result<WinternitzPublicKey, BitVMXError> {
        self.key_manager.derive_winternitz(
            message_bytes, 
    ...
    pub fn derive_winternitz_sha256_keys(&mut self, size_in_bytes: usize, quantity: u32) -> Result<Vec<WinternitzPublicKey>, BitVMXError> {
        self.derive_winternitz_keys(size_in_bytes, WinternitzType::SHA256, quantity)
    }

    pub fn derive_winternitz_hash160_keys(&mut self, size_in_bytes: usize, quantity: u32) -> Result<Vec<WinternitzPublicKey>, BitVMXError> {
    ...
    pub fn sign_program(&self, program: &mut Program) -> Result<(), BitVMXError> {
        ...
        for (txname, infos) in protocol.spending_infos()?.iter() {
            for (input_index, spending_info) in infos.iter().enumerate() {
                ...
                for (message, input_key) in spending_info.hashed_messages().iter().zip(spending_info.input_keys().iter()) {
                    let signature = match spending_info.sighash_type() {
                        SighashType::Ecdsa(sighash_type) => {
                            let signature = self.key_manager.sign_ecdsa_message(message, input_key)?;
                            Signature::Ecdsa(bitcoin::ecdsa::Signature{ signature, sighash_type: *sighash_type })
                        },
                        SighashType::Taproot(sighash_type) => {
                            let signature = self.key_manager.sign_schnorr_message(message, input_key)?;
                            Signature::Taproot(bitcoin::taproot::Signature{ signature, sighash_type: *sighash_type })
                        },
                    };
    ...
    fn derive_winternitz_keys(&mut self, size_in_bytes: usize, key_type: WinternitzType, quantity: u32) -> Result<Vec<WinternitzPublicKey>, BitVMXError> {
        let mut keys = Vec::new();
        for _ in 0..quantity {
            let index = self.winternitz_index.next();
            let pk = self.key_manager.derive_winternitz(size_in_bytes, key_type, index)?;
            keys.push(pk);
        }
