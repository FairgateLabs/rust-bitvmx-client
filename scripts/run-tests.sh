...
if [[ "$NIGHTLY" == "true" ]]; then
    ...
    if [[ -f "run_sequential_tests.sh" ]] && [[ -z "$CI" ]] && [[ -z "$GITHUB_ACTIONS" ]]; then
        ...
        REGTEST_TESTS=(
            ...
            "storage_failure_is_reported_to_l2_and_propagates:5m"
            "bitcoin_outage_is_reported_once_and_the_node_keeps_ticking:5m"
        )
        ...
        for test_spec in "${REGTEST_TESTS[@]}"; do
            ...
            if timeout "$timeout_duration" cargo test "$test_name" --release --features testpanic -- --exact --test-threads=1 --nocapture --ignored; then
